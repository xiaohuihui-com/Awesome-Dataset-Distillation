...
from utils.visual import Plotter
...
def main(args, repeat=1):
    ...
    if opt.dataset.lower() == 'cifar100':
        opt.num_classes = 100
    elif opt.dataset.lower() == 'mnist' or 'svhn':
        opt.channel = 1
        opt.size = 28
    else:
        opt.num_classes = 10
    ...
    for i in range(repeat):
        ...
        best_acc, acc = trainer(opt, model, train_loader, val_loader, nclass, criterion, optimizer, scheduler, device,
                                logger, plotter)
